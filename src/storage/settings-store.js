...
export async function getSetting(key, module = 'global', tenantId = null) {
  const row = await queryOne(
    'SELECT value FROM settings WHERE key = $1 AND module = $2 AND (tenant_id = $3 OR ($3 IS NULL AND tenant_id IS NULL))',
    [key, module, tenantId]
  );
  return row?.value ?? null;
}

export async function getAllSettings(module = 'global', tenantId = null) {
  const rows = await queryRows(
    'SELECT key, value FROM settings WHERE module = $1 AND (tenant_id = $2 OR ($2 IS NULL AND tenant_id IS NULL))',
    [module, tenantId]
  );
  return Object.fromEntries(rows.map(r => [r.key, r.value]));
}

export async function getModuleSettings(module, tenantId = null) {
  return getAllSettings(module, tenantId);
}

export async function setSetting(key, value, module = 'global', tenantId = null) {
  await query(`
...
export async function setSettings(obj, module = 'global', tenantId = null) {
  for (const [key, value] of Object.entries(obj)) {
    await setSetting(key, value, module, tenantId);
  }
}

export async function deleteSetting(key, module = 'global', tenantId = null) {
  await query('DELETE FROM settings WHERE key = $1 AND module = $2 AND (tenant_id = $3 OR ($3 IS NULL AND tenant_id IS NULL))', [key, module, tenantId]);
}
