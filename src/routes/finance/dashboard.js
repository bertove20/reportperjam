...
export default async function (app) {
  app.get('/api/finance/dashboard', async (request) => {
    ...
    const byBrand = await queryRows(`
      ...
      WHERE b.is_active = 1 ${divFilter.replace('division_id', 'b.division_id')}
    ...
    const balances = await queryRows(`
      SELECT pm.id, pm.name, pm.current_balance, pm.currency, bk.name as bank_name
      FROM payment_methods pm
      JOIN banks bk ON pm.bank_id = bk.id
      WHERE pm.is_active = 1 ORDER BY bk.name, pm.name
    `);
